...
sudo crictl --runtime-endpoint unix:///run/containerd/containerd.sock ps  | grep k8s_kube-apiserver
CONTAINER_ID=$(sudo crictl --runtime-endpoint unix:///run/containerd/containerd.sock ps | grep k8s_kube-apiserver | awk '{ print $1 }')
echo $CONTAINER_ID
